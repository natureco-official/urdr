...
const manifest = {
  ...
  long_description: 'Urðr gives Claude a persistent, auditable memory: 4-root Markdown tree backed by a hash-chained event log. The Context Pack compiles the whole tree into a ~375-token session brief (measured 93–227× cheaper than reading root files). 14 tools: search, ask, path, related, read, map, context, report, append, lint, compiler plans, forgetting with verified scrubs. Everything is deterministic and local — no vector DB, no embeddings, no network calls.',
